              ...
              <tbody>
                {specs.map((s, i) => (
                  <tr key={i} className="border-b last:border-0">
                    <th className="w-1/3 p-3 font-semibold text-gray-800">
                      {s.label}
                    </th>
        ...
        {gallery.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {gallery.map((src, i) => (
              <div
                key={i}
                className="relative aspect-square overflow-hidden rounded-xl ring-1 ring-black/5 bg-white"
              >
                <Image
                  src={src}
                  alt={${name} foto ${i + 1}}
        ...
        {videos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {videos.map((v, i) => (
              <div
                key={i}
                className="rounded-xl overflow-hidden ring-1 ring-black/5 bg-white"
              >
      ...
      <section className="mt-10">
        ...
        <div className="rounded-2xl p-5 bg-white ring-1 ring-black/5">
          <p className="text-gray-700">
            Precisa de ajuda sobre {name}? Fale conosco em{" "}
            <a href="/#suporte" className="text-blue-700 underline">
              /suporte
            </a>.
          </p>
        </div>
