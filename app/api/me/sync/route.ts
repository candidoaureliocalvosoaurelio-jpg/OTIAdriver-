...
export async function POST(req: Request) {
  try {
    ...
    // 3) Se não achou pelo payment_id, pega o último aprovado (mesmo que applied_at já tenha sido marcado)
    if (!pay) {
    ...
    // se não tem pagamento aprovado real, não aplica
    ...
    // marca applied_at (se já estiver marcado, não tem problema)
    const mark = await sb
      .from("mp_payments")
    ...
    if (mark.error) {
      ...
      // mesmo se falhar, o profiles já foi atualizado
    }
